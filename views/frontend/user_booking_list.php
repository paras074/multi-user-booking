...
if(! is_user_logged_in()){
	...
	/*	if (  $access_granted ) 
	 ...
	 $user_id=$current_user->ID;
				$users_data = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix . "user_booking_setting Where user_id=".$user_id
);
			//print_r($users_data);
				 $users_booking_list = $wpdb->get_results
			(
				"SELECT * FROM ".$wpdb->prefix . "user_booking Where user_booking_clander_id=".$users_data->id
			);
		...
		<div class="col-lg-12">
			<table class="table table-striped table-bordered table-hover table-margin-top" id="cb_bookings_data">
				...
				<?php
					...
					foreach($users_booking_list as $users):
